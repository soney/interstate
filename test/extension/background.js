					...
					if(source.tabId === debuggerId.tabId) {
						...
						if(request.forbidden_tokens.length > 0) {
							if(method === "HeapProfiler.addHeapSnapshotChunk") {
								...
								for(i = 0; i<bad_nodes.length; i++) {
									var bn = bad_nodes[i];
									bn.explored = true;

									var queue = [];
									queue.push.apply(queue, bn.incoming_edges);

									while(queue.length > 0) {
										edge = queue.pop();
										if(edge.type === "context" || edge.type === "element" || edge.type === "property" || edge.type === "weak" || edge.type === "shortcut" || edge.type === "internal") {
											node = edge.from_node;
											if(node === computed_nodes[0]) {
												illegal_strs = true;
												break;
											} else {
												if(!node.explored) {
													queue.push.apply(queue, node.incoming_edges);
													node.explored = true;
												}
											}
										}
									...
									for(j = 0; j<num_nodes; j++) {
										delete computed_nodes[i].explored;
									}
								}
								*/
