	...
	proto.get = function(context) {
		...
		if(_.isNumber(basis)) {
			...
			_.times(basis, function(index) {
				var dict = cjs.create("red_stateful_obj", {defer_statechart_invalidation: true, direct_protos: self._template});
			...
			rv = _.map(basis, function(basis_obj) {
				var dict = cjs.create("red_stateful_obj", {defer_statechart_invalidation: true});
				dict._set_direct_protos(self.get_template());
