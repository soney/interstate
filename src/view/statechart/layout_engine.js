...
(function (my) {
	...
	proto.get_background_css = function () {
		...
		var weaved = [];
		for (var i = 0; i<bg_images.length; i += 1) {
		...
		for (var i = 0; i<columns.length; i += 1) {
			...
			if (column.lr === "l" || column.lr === "r") { //it's a transition pole
				...
				if (column.lr === "l") {
					...
					for (var row = column_values.length-1; row>=column.depth; row -= 1) {
					...
					for (var row = column.depth; row < column_values.length; row += 1) {
						var cell = column_values[row];
						if (cell === state) {
							wing_start_x = x;
							wing_start_y = y;
