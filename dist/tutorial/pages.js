	...
	return [
		{
			...
			editor: {
				text: "<p>Each of <var>my_circle</var>'s properties control some aspect of how <var>my_circle</var> is displayed on screen. To <em>override</em> an inherited property (make it your object's own so you can change it), click its name. To cancel overriding a property, right click that property's name and select delete.</p>" +
						...
						"<div class='note'>note: you can undo/redo edits by clicking menu in the top right corner of the editor or by pressing <kbd>ctrl+z</kbd> and <kbd>ctrl+y</kbd> (<kbd>&#8984+Z</kbd> and <kbd>&#8984+Y</kbd> on a Mac)</div>"
			}
		}, {
			editor: {
			...
			editor: {
				text: "<p>The space near the top of <var>my_circle</var> is dedicated to maintaining a <em>state machine</em>. A state machine tracks the status of <var>my_circle</var> at any given time. It consists of <em>states</em> to represent different statuses and <em>transitions</em> to specify how to go between states.</p>" +
						"<p>Let's make <var>my_circle</var> red when the user hovers it. We can represent this behavior with two states (one 'not hovering' and one for hover)</p>" +
			...
			editor: {
				text: "<p>To rename a state, click on its name, type a new name, and press <kbd>enter</kbd>.</p>" +
			...
			editor: {
				text: "<p>To specify that a property should have a value in a state, click the grey circle for that property's row and that state's column.</p>" + 
						"<div class='directive'>Set <var>fill</var> to <code>'black'</code> in <var>not_hover</var> and to <code>'yellow'</code> in <var>hover</var>.</div></p>" +
						"<div class='note'>note: the value of <var>fill</var> in the start transition (black dot) will still be <code>'yellow'</code> but it immediately changes to 'black' when it enters the 'not_hover' state.</div>"
			}
		}, {
			editor: {
				text: "<p>Next, we need to add <em>transitions</em> to specify when <var>my_circle</var> changes state, right click the state you are transitioning from, select 'Add Transition', and click the state to transition to.</p>" +
						"<div class='directive'>Add a transition from <var>init</var> to <var>pressed</var>."
			}
		}, {
			editor: {
			...
			editor: {
				text: "<div class='directive'>Add a transition from <var>hover</var> to <var>not_hover</var> with the event <code>on('mouseout', this)</code>. Play around with your app.</div>"
			}
		}, {
			editor: {
			...
			editor: {
				text: "<p>Now, <var>my_circle</var> represents a set of objects instead of a single object (as indicated by the stack behind <var>my_circle</var>). Because the array that you just entered has three values (<code>'red'</code>, <code>'green'</code>, and <code>'blue'</code>), there are three copies of <var>my_circle</var>. Every copy also has two extra properties: <var>my_copy</var> &amp; <var>copy_num</var>. <var>my_copy</var> represents that copy's item (in this case, <code>'red'</code>, <code>'green'</code>, or <code>'blue'</code>). <var>copy_num</var> is that item's index (in this case, <code>0</code>, <code>1</code>, or <code>2</code>).</p>" +
					...
					"<div class='directive'>Right now, all of the copies of <var>my_circle</var> are stacked on top of each other. Change that by setting <var>cy</var> to <code>90 + copy_num * r</code>. Then, set <var>fill</var> to <code>my_copy</code></div>" +
					"<div class='note'>note: you can make cells larger while editing them by clicking and dragging their bottom right corner.</div>"
			}
		}, {
			editor: {
				text: "<p>Although the edits you make affect every copy of <var>my_circle</var>, you can only view the <em>current values</em> of its properties one at a time. Right now, you are looking at the first copy.</p>" +
			...
			editor: { // The find function can be used to find objects.
				text: "<p>The built-in <var>find(root)</var> function can be used to query objects. <var>find</var> accepts a <var>root</var> object to start at and returns a special 'query' object. Find uses 'chaining' to filter its query. For example, <code>find(my_circle).in_state('hover').eq(0)</code> returns every copy of <var>my_circle</var> in the <var>hover</var> state and returns the first one.</p>"
						+ "<div class='directive'>Create a new object on screen that inherits from <var>shape.rect</var>. Set its <var>following</var> property to the expression <code>find(my_circle).in_state('hover').eq(0)</code> and make t folow the highlighted object by settings <var>y</var> property to <var>following.cy-following.r</var>.</div>"
			}
		}, {
			editor: {
